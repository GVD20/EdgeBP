 ...
 * @LastEditTime: 2024-10-13 21:33:38
 * @FilePath: \max30102_test\components\max30102\src\blood.c
 * @Description:
 */
#include "blood.h"
#include "esp_log.h"
...
{
    static const char *TAG = "PPG_DATA";
    ...
    while (g_fft_index < FFT_N)
    {
        while (gpio_get_level(sens->int_pin) == 0)
        {
            ...
            if (g_fft_index < FFT_N)
            {
                ...
                // 通过ESP-IDF的日志接口输出原始PPG数据
                ESP_LOGI(TAG, "PPG_RAW,%u,%u,%u", g_fft_index, fifo_red, fifo_ir);
